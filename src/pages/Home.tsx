...
const highlights = [
  {
    ...
    text: (
      <>
        Co-author of{" "}
        <em className="text-navy-800/90">Powerstones: Letters to a Goddess</em>{" "}
...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="relative py-28 md:py-44 px-6 text-center overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative">
          ...
          <div className="fade-in fade-in-delay-2">
            <blockquote className="relative text-lg sm:text-xl md:text-2xl text-navy-700/75 italic font-light leading-relaxed max-w-2xl mx-auto mb-12 px-6">
      ...
      {/* Highlights */}
      <section className="bg-navy-50/50 py-20 md:py-28 px-6 relative">
        <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-navy-800 text-center mb-16 fade-in">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {highlights.map((item, i) => (
              <div
                key={i}
                className={`fade-in fade-in-delay-${i + 1} group bg-white rounded-3xl p-8 shadow-sm border border-navy-800/5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden`}
              >
                <div
                  className={`absolute top-0 left-0 right-0 h-1 ${item.accent === "gold" ? "bg-gradient-to-r from-gold-dark via-gold to-gold-light" : "bg-gradient-to-r from-navy-800 via-navy-600 to-navy-400"}`}
                />

                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${item.accent === "gold" ? "bg-gold/10" : "bg-navy-800/10"}`}
                >
                  <svg
                    className={`w-7 h-7 ${item.accent === "gold" ? "text-gold-dark" : "text-navy-800"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={1.5}
                ...
                <div className="pt-5 border-t border-navy-800/5">
                  <span className="text-3xl font-bold text-navy-800/90">
                    {item.stat}
                  </span>
      ...
      <div className="relative h-16 md:h-24 bg-navy-50/50 -mb-1">
        <svg
          viewBox="0 0 1440 96"
          fill="none"
          className="absolute bottom-0 w-full h-full"
      ...
      {/* YouTube Section */}
      <section className="relative py-20 md:py-28 px-6 bg-navy-900 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
      ...
      <section className="relative py-20 md:py-28 px-6 text-center overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto relative fade-in">
          ...
          <p className="text-navy-700/60 text-lg mb-10">
